...
func (v *ValidatingAdmission) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	for _, config := range configs.Items {
		if !validateCustomizationRule(config, *configuration) {
			return admission.Denied(fmt.Sprintf("Conflicting with existing ResourceInterpreterCustomization(%s)", config.Name))
		}
	}
